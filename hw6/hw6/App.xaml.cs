...
namespace hw6 {
    ...
    sealed partial class App : Application {
        ...
        private void LoadDatabase() {     // Get a reference to the SQLite database
            conn = new SQLiteConnection("todoitem.db");
            ...
            using (var statement = conn.Prepare("SELECT COUNT(*) FROM TodoItem;")) {
                statement.Step();
                if ((Int64)statement[0] == 0) {
                    using (var insert = conn.Prepare("INSERT INTO TodoItem (Name, Detail, DueDate, Finished) VALUES (\"test1\", \"test one\", \"2016/5/3\", 1), (\"test2\", \"test two\", \"2016/8/1\", 0);")) {
                        insert.Step();
                    }
                }
            }
